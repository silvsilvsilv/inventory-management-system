...
<body class="bg-gray-100 text-gray-800">
		...
		<main class="flex-1 p-8">
			...
			<section class="bg-white p-6 rounded-xl shadow">
				<div class="flex justify-between items-center mb-4">
					...
					<button onclick="openModal('addModal')" class="bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700">+
						Add Product</button>
				</div>
	...
	<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center hidden z-50">
		<div class="bg-white p-6 rounded-lg w-full max-w-md shadow-lg">
			...
			<div class="flex justify-end space-x-2">
				<form action="{{route('manager.delete_product')}}" method="POST">
					@csrf
					@method('DELETE')

...
<header class="sticky top-0 z-50 bg-white shadow-sm backdrop-blur-sm backdrop-saturate-150">
  <nav class="max-w-[1200px] mx-auto flex items-center justify-between px-6 py-4">
    ...
    <ul class="hidden md:flex space-x-10 font-medium text-gray-600">
      <li><a href="{{route('manager.dashboard')}}" class=" hover:text-blue-600 transition-colors
          duration-300">Inventory</a></li>
      <li><a href="{{route('manager.sales')}}" class="hover:text-blue-600 transition-colors duration-300">Sales</a></li>
      <li><a href="{{route('manager.staff')}}" class="hover:text-blue-600 transition-colors duration-300">Staff</a></li>
      <li><a href="{{route('manager.logs')}}" class="hover:text-blue-600 transition-colors duration-300">Activity
          Log</a></li>
    </ul>

    <form method="POST" action="{{ route('logout') }}" class="md:inline-b">
      @csrf
      <button type="submit"
        class="bg-gray-900 text-white py-2 px-6 rounded-lg font-semibold hover:bg-gray-800 transition-smooth focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
        Sign Out
      </button>
    </form>

    <button id="mobile-menu-btn" aria-label="Open menu"
      class="md:hidden focus:outline-none focus:ring-2 focus:ring-gray-900 rounded">

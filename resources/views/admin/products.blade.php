...
<body class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-6xl mx-auto px-6 py-20">
      ...
      <form method="GET" action="{{ route('admin.products') }}" class="mb-4 flex items-center">
            ...
            <button type="button" class="ml-2 bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 whitespace-nowrap" id="createCategory" onclick="openModal('addProductModal')">
                Add New Product
    ...
    <div id="editProductModal" class="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center hidden z-50">
        <div class="bg-white p-6 rounded-lg w-full max-w-lg shadow-xl">
            ...
            <form class="grid grid-cols-1 gap-4" action="{{ route('admin.edit_product') }}" method="POST">
                ...
                <input type="text" id="editProductName" placeholder="Product Name" class="border p-2 rounded" required name="name"/>
                <select id="editProductCategory" name="category_id" class="border p-2 rounded" required name="category_id">
                    <option value="" disabled>Select Category</option>
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" class="capitalize">{{ $category->name }}</option>
                    @endforeach
                </select>
                <input type="number" id="editProductPrice" placeholder="Price" class="border p-2 rounded" required name="price"/>
                <input type="number" id="editProductStock" placeholder="Stock" class="border p-2 rounded" required name="stock"/>
                <div class="flex justify-end space-x-2">
